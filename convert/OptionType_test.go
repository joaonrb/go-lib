...
func TestOptionTypeValueShouldConvertCorrectlyWhenDoingCorrectOperation(t *testing.T) {
	...
	option := convert.OptionType[string, int](stringOption).Then(mustIntConverter)
	require.IsTypef(t, types.Value[int]{}, option, "option is expected to Value[int], got %T instead", option)
...
func TestOptionTypeValueShouldNotConvertWhenDoingIncorrectOperation(t *testing.T) {
	...
	option := convert.OptionType[string, int](stringOption).Then(mustIntConverter)
	require.IsTypef(t, types.Nothing[int]{}, option, "option is expected to Nothing[int], got %T instead", option)
}
...
func TestOptionTypeOKStringRepresentationShouldHaveTheType(t *testing.T) {
	...
	assert.Equal(t, "Value[int, string]{This: 10}", fmt.Sprint(convert.OptionType[int, string](intOption)))
	var stringOption types.Option[string] = types.Value[string]{This: "10"}
	assert.Equal(t, "Value[string, int]{This: \"10\"}", fmt.Sprint(convert.OptionType[string, int](stringOption)))
	var errorResult types.Option[int] = types.Nothing[int]{}
	assert.Equal(t, "Nothing[int, string]{}", fmt.Sprint(convert.OptionType[int, string](errorResult)))
}
